...
def pyomo_model_prep(data, timesteps, window_start, window_end):
    ...
    commodity["support_timeframe"] = commodity.index.get_level_values(
        "support_timeframe"
    )
    process["support_timeframe"] = process.index.get_level_values("support_timeframe")
    if m.mode["tra"]:
        transmission["support_timeframe"] = transmission.index.get_level_values(
            "support_timeframe"
        )
    if m.mode["sto"]:
        storage["support_timeframe"] = storage.index.get_level_values(
            "support_timeframe"
        )
    ...
    m.r_in_dict = (
        data["process_commodity"].xs("In", level="Direction")["ratio"].to_dict()
    )
    m.r_out_dict = (
        data["process_commodity"].xs("Out", level="Direction")["ratio"].to_dict()
    )
    ...
    if m.mode["int"]:
        ...
        for index in tuple(pro_const_cap.index):
            ...
            if (
                not stf_process["cap-up"].max(axis=0)
        ...
        process["discount"] = m.global_prop.xs("Discount rate", level=1).loc[
            m.global_prop.index.min()[0]
        ]["value"]
        process["stf_min"] = m.global_prop.index.min()[0]
        process["stf_end"] = (
            m.global_prop.index.max()[0]
            + m.global_prop.loc[
                (
                    max(commodity.index.get_level_values("support_timeframe").unique()),
                    "Weight",
                )
            ]["value"]
            - 1
        ...
        process.loc[
            (process["overpay-factor"] < 0) | (process["overpay-factor"].isnull()),
            "overpay-factor",
        ] = 0
        ...
        commodity["eff-distance"] = commodity["stf_dist"].apply(effective_distance, m=m)
        commodity["cost_factor"] = (
            commodity["discount-factor"] * commodity["eff-distance"]
        )
        process["stf_dist"] = process["support_timeframe"].apply(stf_dist, m=m)
        process["discount-factor"] = process["support_timeframe"].apply(
            discount_factor, m=m
        )
        process["eff-distance"] = process["stf_dist"].apply(effective_distance, m=m)
        process["cost_factor"] = process["discount-factor"] * process["eff-distance"]
        ...
        if m.mode["tra"]:
            ...
            for index in tuple(tra_const_cap.index):
                stf_transmission = transmission.xs(
                    (index[1], index[2], index[3], index[4]), level=(1, 2, 3, 4)
                )
                if (
                    not stf_transmission["cap-up"].max(axis=0)
                    == tra_const_cap.loc[index]["inst-cap"]
                ):
                    tra_const_cap = tra_const_cap.drop(index)
            # derive invest factor from WACC, depreciation and
            # discount untility
            transmission["discount"] = m.global_prop.xs("Discount rate", level=1).loc[
                m.global_prop.index.min()[0]
            ]["value"]
            transmission["stf_min"] = m.global_prop.index.min()[0]
            transmission["stf_end"] = (
                m.global_prop.index.max()[0]
                + m.global_prop.loc[
                    (
                        max(
                            commodity.index.get_level_values(
                                "support_timeframe"
                            ).unique()
                        ),
                        "Weight",
            ...
            transmission["invcost-factor"] = transmission.apply(
                lambda x: invcost_factor(
                    x["depreciation"],
                    x["wacc"],
                    x["discount"],
            ...
            # discount untility
            transmission["overpay-factor"] = transmission.apply(
                lambda x: overpay_factor(
                    x["depreciation"],
                    x["wacc"],
                    x["discount"],
            ...
            # Derive multiplier for all energy based costs
            transmission.loc[
                (transmission["overpay-factor"] < 0)
                | (transmission["overpay-factor"].isnull()),
                "overpay-factor",
            ] = 0
            transmission["stf_dist"] = transmission["support_timeframe"].apply(
                stf_dist, m=m
            )
            transmission["discount-factor"] = transmission["support_timeframe"].apply(
                discount_factor, m=m
            )
            transmission["eff-distance"] = transmission["stf_dist"].apply(
                effective_distance, m=m
            )
            transmission["cost_factor"] = (
                transmission["discount-factor"] * transmission["eff-distance"]
            )
        # storage mode
        if m.mode["sto"]:
            # modify sto_const_cap_c and sto_const_cap_p for intertemporal mode
            for index in tuple(sto_const_cap_c.index):
                stf_storage = storage.xs(
                    (index[1], index[2], index[3]), level=(1, 2, 3)
                )
                if (
                    not stf_storage["cap-up-c"].max(axis=0)
            ...
            for index in tuple(sto_const_cap_p.index):
                ...
                if (
                    not stf_storage["cap-up-p"].max(axis=0)
            ...
            # derive invest factor from WACC, depreciation and
            # discount untility
            storage["discount"] = m.global_prop.xs("Discount rate", level=1).loc[
                m.global_prop.index.min()[0]
            ]["value"]
            ...
            storage["invcost-factor"] = storage.apply(
                lambda x: invcost_factor(
                    x["depreciation"],
                    x["wacc"],
                    x["discount"],
            ...
            storage["overpay-factor"] = storage.apply(
                lambda x: overpay_factor(
                    x["depreciation"],
                    x["wacc"],
                    x["discount"],
            ...
            storage.loc[
                (storage["overpay-factor"] < 0) | (storage["overpay-factor"].isnull()),
                "overpay-factor",
            ] = 0

            storage["stf_dist"] = storage["support_timeframe"].apply(stf_dist, m=m)
            storage["discount-factor"] = storage["support_timeframe"].apply(
                discount_factor, m=m
            )
            storage["eff-distance"] = storage["stf_dist"].apply(effective_distance, m=m)
            storage["cost_factor"] = (
                storage["discount-factor"] * storage["eff-distance"]
            )
    else:
        # for one year problems
        process["invcost-factor"] = process.apply(
            lambda x: invcost_factor(x["depreciation"], x["wacc"]), axis=1
        )
        ...
        if m.mode["dpf"]:
            ...
            for t in m.transmission_dc_dict["reactance"]:
                m.transmission_dict["inv-cost"][t] = (
                    2 * m.transmission_dict["inv-cost"][t]
                )
                m.transmission_dict["fix-cost"][t] = (
                    2 * m.transmission_dict["fix-cost"][t]
                )
    ...
    m.mode["exp"]["pro"] = identify_expansion(
        pro_const_cap["inst-cap"], process["inst-cap"].dropna()
    )
    m.pro_const_cap_dict = pro_const_cap["inst-cap"].to_dict()

    if m.mode["tra"]:
        m.mode["exp"]["tra"] = identify_expansion(
            tra_const_cap["inst-cap"], transmission["inst-cap"].dropna()
        )
